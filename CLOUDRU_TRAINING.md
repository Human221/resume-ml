...
2. Создайте виртуальную машину с GPU:
   - **ОС**: Ubuntu 24.04 (рекомендуется) или 22.04
   - **GPU**: Выберите GPU с минимум 16GB памяти (например, NVIDIA A100, V100)
   - **RAM**: Минимум 32GB (рекомендуется 64GB для больших моделей)
   - **Диск**: Минимум 100GB (рекомендуется 256GB)
   - **vCPU**: 8+ ядер
   - **Гарантированная доля vCPU**: 100%

**Рекомендуемая конфигурация для быстрого обучения:**
- vCPU: 8
- RAM: 64 ГБ
- Образ: Ubuntu 24.04
- Гарантированная доля vCPU: 100%
- Размер диска: 256 ГБ
- GPU: NVIDIA A100 или эквивалент с 16GB+ памяти
